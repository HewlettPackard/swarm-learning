...
|`WAIT FOR TASKRUNNER <trName : string>`|Waits for the Taskrunner to complete its current task.<br>`trName`: Specifies the unique name of the Taskrunner.<br>WAITING FOR TASKRUNNER TO COMPLETE - Maximum wait time is : <SWCI_TASK_MAX_WAIT_TIME>.<br>If SWCI_TASK_MAX_WAIT_TIME is not set, the default value is 120 mins.<br>Prints # (a progress indicator) periodically until the task completes.<br>When the task completes, the final state of the task is printed.<br>If the maximum wait time is reached, the following warning message is displayed:<br>WARNING - Maximum configured SWCI wait time is over|
