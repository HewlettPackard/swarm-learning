...
def main():
    ...
    config = LoraConfig(task_type=TaskType.SEQ_CLS, r=8, lora_alpha=32, lora_dropout=0.1, bias="none")
    ...
    def compute_metrics(eval_pred):
        ...
        compute_metrics.predictions = predictions
    ...
    compute_metrics.predictions = None
    compute_metrics.labels = None
    ...
    print(f"\n[INFO] Full fine-tuning - PEFT Huggingface Trainer training completed on {device}")
    ...
    # Save the PEFT model instead of full model
    model.save_pretrained(f"{output_dir}/final")
    tokenizer.save_pretrained(f"{output_dir}/final")
    
    model_save_end = time.time()
    save_duration = model_save_end - model_save_start
    
    # Calculate total time
    total_time = time.time() - start_time
    
    print(f"[INFO] Model saved in {save_duration:.2f} seconds")
    print(f"[INFO] Total runtime: {total_time:.2f} seconds ({total_time/60:.2f} minutes)")
    print(f"[INFO] PEFT model saved successfully!")
